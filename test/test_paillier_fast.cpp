...
TEST_CASE(STR(PAILLIER_CLASS)) {
    ...
    SECTION("sanity checks") {
        ...
        REQUIRE_THROWS_AS( pai1.zero_ciphertext(), BaseException );
    ...
    SECTION("different key sizes") {
        const size_t keys[] = {1024, 2048, 3072, 4096, 7680, 0};
        for(auto k = keys; *k > 0; k++) {
            ...
            REQUIRE( pai.decrypt(pai.zero_ciphertext()) == 0 );
    ...
    SECTION( "test with random numbers" ) {
        for(int i = 0; i < 20; i++) {
            Integer r = Random::instance().rand_int(paillier.plaintext_upper_boundary());
...
TEST_CASE(STR(PAILLIER_CLASS)"::generate_keys") {
    PAILLIER_CLASS paillier(keysize);
    paillier.generate_keys();
    const PrivateKey priv = paillier.get_priv();
    const PublicKey pub = paillier.get_pub();

    SECTION( "p and q are prime" ) {
        REQUIRE( priv.p.is_prime() );
        REQUIRE( priv.q.is_prime() );
    }

    SECTION( "(g^n)^a = 1 mod n^2" ) {
        REQUIRE( pub.g.pow_mod_n(pub.n * priv.a, pub.n * pub.n) );
    }

    SECTION( "a is divisor of lambda" ) {
        Integer lambda = Integer::lcm(priv.p - 1, priv.q - 1);
        REQUIRE( ((lambda / priv.a) * priv.a) == lambda );
        REQUIRE( lambda % priv.a == 0 );
    }
}
...
TEST_CASE(STR(PAILLIER_CLASS)"::Ciphertext") {
    ...
    SECTION( "test with random numbers" ) {
        for(int u = 0; u < 10; u++) {
            Integer a =  Random::instance().rand_int_bits(30),
                    b = -Random::instance().rand_int_bits(30);
